                            ...
                            <?php if(count($balance_all)){ ?>
                                <?php foreach($balance_all as $key => $value){ ?>
                                    <tr>
                                        ...
                                        <td><?= $value['addr'] ?:'<button class="btn btn-info btn-sm" onclick="generate_address('."'".$value['name']."'".')">生成'.$value['name'].'地址</button>' ?></td>
                                        <td>
                                            <button id="recharge" type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#myModal" data-coin="<?= $value['name'] ?>" data-addr="<?= $value['addr'] ?>" data-user="<?= $user->id ?>" data-title="充币">充币</button>
                                            <button id="deduction" class="btn btn-info btn-sm" data-toggle="modal" data-target="#myModal" data-coin="<?= $value['name'] ?>" data-addr="<?= $value['addr'] ?>" data-user="<?= $user->id ?>" data-title="扣币">扣币</button>
                                            <button id="freeze" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#myModal" data-coin="<?= $value['name'] ?>" data-addr="<?= $value['addr'] ?>" data-user="<?= $user->id ?>" data-title="冻结">冻结</button>
                                            <button id="unfreeze" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#myModal" data-coin="<?= $value['name'] ?>" data-addr="<?= $value['addr'] ?>" data-user="<?= $user->id ?>" data-title="解冻">解冻</button>
                                            <a href="<?= Url::to(['coinlock','uid'=>$user['id'],'coin_symbol'=>$value['name']])?>""><span class="btn btn-info btn-sm">认购</span></a>
            ...
            <form action="<?= Url::to(['update-balance'])?>" method="post" class="form-horizontal" role="form" id="form">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <input type="text" class="form-control value" name="memo" placeholder="请输入备注">
                        </div>
                    </div>                    
                    <!-- 用户id -->
