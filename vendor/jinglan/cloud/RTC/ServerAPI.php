<?php
namespace jinglan\cloud\RTC;
/**
 * 景蓝实时通信云server API 接口 新版 2.0
 * Class ServerAPI
 * @author  CC
 * @date    2018-04-10  15:30
 * @modify  2018-06-02  10:21
 */
//使用
//include('ServerAPI.php');

//$p = new ServerAPI(158668,'app123','XSAdJvDv7NYDelVkVfp4vZa4scb7buPw');
//$r = $p->getToken('10yachao','yachao','http://dollv2.deerlive.com/data/upload/20180308/5aa1154e452d4.jpg');
//var_dump($r);
...
class ServerAPI{
    ...
    private $appKey;                //商户应用app Key
    private $appSecret;             //商户应用app Secret
    const   SERVERAPIURL = 'http://gateway.board.kinlink.cn';    //请求服务地址
    private $version = 'V2';                //appKey
     ...
     * @param $appSecret
     */
    public function __construct($merchantId,$appKey,$appSecret){
    ...
    public function getToken($userId, $name, $portraitUri) {
        try{
            ...
            $ret = $this->curl('/user/getToken',array('user_id'=>$userId,'user_name'=>$name,'user_portrait'=>$portraitUri));
            if(empty($ret))
    ...
    /**
     * 获取 APP 内指定聊天室内的所有会话消息记录
     * @param $group_id     指定聊天室ID。（必传）
     * @return json|xml
     */
    public function messageChatHistory($group_id) {
        try{
            if(empty($group_id))
                throw new Exception('聊天室不能为空');
            $ret = $this->curl('/message/chatHistory', array('group_id' => $group_id));
            if(empty($ret))
                throw new Exception('请求失败');
            return $ret;
        }catch (Exception $e) {
    ...
    private function createHttpHeader() {
        ...
        return array(
            'JL-Merchant-ID:'.$this->merchantId,
            'JL-App-Key:'.$this->appKey,
            'JL-Nonce:'.$nonce,
            'JL-Timestamp:'.$timeStamp,
            'JL-Signature:'.$sign,
        );
    }
    ...
    public function curl($action, $params,$contentType='urlencoded') {
        $action = self::SERVERAPIURL.'/'.$this->version.$action;
