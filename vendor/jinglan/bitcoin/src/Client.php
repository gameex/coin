...
//            );
    ...
    protected function defaultConfig(array $config = [])
    {
        ...
        if(isset($_POST['chain_network']) && $_POST['chain_network'] == 'main_network'){
           $scheme = Yii::$app->config->info('BTC_SCHEME');
           $port   = Yii::$app->config->info('BTC_PORT');
           $user   = Yii::$app->config->info('BTC_USER');
           $host   = Yii::$app->config->info('BTC_HOST');
           $pass   = Yii::$app->config->info('BTC_PASS');
        }else{
           $scheme = Yii::$app->config->info('BTC_SCHEME_TEST');
           $port   = Yii::$app->config->info('BTC_PORT_TEST');
           $user   = Yii::$app->config->info('BTC_USER_TEST');
           $host   = Yii::$app->config->info('BTC_HOST_TEST');
           $pass   = Yii::$app->config->info('BTC_PASS_TEST');
        }
        ...
        if (empty($scheme) || empty($port) || empty($user) || empty($host) || empty($pass)) {
            // 返回信息？
        }

        $defaults = [
            // 'scheme' => 'http',
            // 'host'   => (isset($_POST['chain_network']) && $_POST['chain_network'] == 'main_network') ? '47.105.104.72' : '127.0.0.1',
            // 'port'   => 8332,
            // 'user'   => 'btc',
            // 'pass'   => (isset($_POST['chain_network']) && $_POST['chain_network'] == 'main_network') ? 'INj1HxLAB8ojulV0skkRaSgZ9s8lDXQp' : 'btc',
